...
static inline void pullup(uint8_t PIN) {
    digitalWrite(PIN, HIGH);
}
static inline void noPullup(uint8_t PIN) {
    digitalWrite(PIN, LOW);
}

static inline void portMode(uint8_t, uint8_t)
...
static inline void portWrite(uint8_t, uint8_t)
    __attribute__((always_inline, unused));
