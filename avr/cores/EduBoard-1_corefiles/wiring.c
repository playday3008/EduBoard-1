...
// 24MHz: An overflow happens every 682.67 microseconds ---> 0.04167, so this
// results in 682 20MHz: An overflow happens every 819.2 microseconds ---> 0,05
// (time of a cycle in micros) * 64 (timer0 tick) * 256 (every 256 ticks timer0
// overflows), so this results in 819 16MHz: An overflow happens every 1024
// microseconds
#if 0
...
// For 16 MHz: 24 (1024 % 1000) gets shifted right by 3 which results in 3
// (precision was lost) For 20 MHz: 819 (819 % 1000) gets shifted right by 3
// which results in 102 (precision was lost) For 24 MHz: 682 (682 % 1000) gets
// shifted right by 3 which results in
...
#        define CORRECT_BRUTE \
            ((2U * 135U * EXACT_REM + EXACT_DEN) / (2U * EXACT_DEN))
...
#endif
{
    ...
    // (volatile variables must be read from memory on every access, so this
    // saves time)
 ...
 */
#    if F_CPU >= 32000000L
    // we need to put this #if here to avoid entering the wrong branch for 32
    // MHz
...
#    else
    ...
    // Shift by 8 to the left (multiply by 256) so t (which is 1 byte in size)
    // can fit in m & t are multiplied by 4 (since it was already multiplied by
    // 256) t is multiplied by 4
    return ((m << 8) + t) * (64 / clockCyclesPerMicrosecond());
#    endif
#endif // 0
}
 ...
 * Assumes a 1, 1.8432, 2, 3.6864, 4, 7.3728, 8, 9.216, 10, 11.0592,
 * 12, 14.7456, 16, 18.432, 20, 24, 25 or 32 MHz clock. */

#define _MORENOP_ ""

/* Link time optimization (LTO for short) has been supported by the IDE since
 * v1.6.11. In Arduino IDE 1.6.11 and newer LTO is enabled by default.  The LTO
 * optimizes the code at link time, making the code (often) significantly
 * smaller without making it "slower" and sometimes destroy acccurate software
 * timings like delayMicroseconds() with lower values. To avoid LTO
 * optimization, the line of delayMicroseconds() definition in arduino.h must be
 * replaced by this: void delayMicroseconds(unsigned int) __attribute__
 * ((noinline)) ;
 */
void delayMicroseconds(unsigned int us) {
    // Question:
    // We multiply `us' by as much as 6 below.  This reduces the available range
    // of us. Updated README to define the safe calling range to 0 .. 10000 us.

    // call = 4 cycles + 1 to 4 cycles to init us(2 for constant delay, 4 for
    // variable,
    ...
    if (us == 0)
        return; // 3 cycles (.1us) on false, which we ignore
...
#    define _MORENOP_ " nop \n\t"
...
#elif F_CPU >= 22118400L
    // this is basically the same as for 11.0592, except multiplying by 6,
    // not 3. the correction factor is the same, but the multiply takes 4 cycles
    // longer.
    ...
    if (us > 16) // = 3 cycles
    {
        ...
        us = (us * 60398UL) >> 16; // x0.9216 us = 29 cycles (60398 = 0.9216 x
                                   // 0x10000L)
    ...
    if (us > 17) // = 3 cycles
    {
        ...
        us = (us * 60398UL) >> 16; // x0.9216 us = 29 cycles (60398 = 0.9216 *
                                   // 0x10000L)
...
#    define _MORENOP_ " nop \n\t"
    ...
    if (us > 15) // = 3 cycles
    {
        ...
        // (50 cycles = 6.78 us) + (12 cycles = 1.63 us) = 8.41 us when 8 us
        // wanted
        us -= 13; // = 2 cycles
    } else {
        // account for the time taken in the preceeding commands.
...
#    define _MORENOP_ " nop \n\t  nop \n\t"
    ...
    // we don't need to multiply here because one request microsecond is exactly
    // one loop cycle
    ...
    // we don't need to multiply here because one request microsecond is almost
    // one loop cycle
    ...
    if (us > 54) // = 3 cycles
    {
        ...
        us = (us * 30199L) >> 16; // x(0.9216/2) us = 29 cycles (30199 = 0.4608
                                  // x 0x10000L)
...
#else
    // for the 1 MHz internal clock (default settings for most ATmega
    // microcontrollers)
    ...
    if (us <= 16)
        return; //= 3 cycles, (4 when true)
    if (us <= 25)
        return; //= 3 cycles, (4 when true), (must be at least 25 if we want to
                //substract 22)

    // compensate for the time taken by the preceeding and next commands (about
    // 22 cycles)
    us -= 22; // = 2 cycles
...
void init() {
    ...
    // This combination is for the ATmega8535, ATmega8, ATmega16, ATmega32,
    // ATmega8515, ATmega162
    TCCR0 |= _BV(CS01) | _BV(CS00);
#    if defined(WGM00) && defined(WGM01) // The ATmega8 doesn't have WGM00 and
                                         // WGM01
...
//  Timer 2 not finished (may not be present on this CPU)
    ...
    TCCR4B |= _BV(CS42) | _BV(CS41) | _BV(CS40); // Set timer 4 prescale factor
                                                 // to 64
    TCCR4D |= _BV(WGM40); // Put timer 4 in phase- and frequency-correct PWM
                          // mode
