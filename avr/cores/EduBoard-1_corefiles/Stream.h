...
class Stream : public Print {
  ...
  public:
    ...
    void setTimeout(unsigned long timeout); // sets maximum milliseconds to wait
                                            // for stream data, default is 1
                                            // second
    ...
    bool find(char *target, size_t length); // reads data from the stream until
                                            // the target string of given length
                                            // is found
    ...
    bool findUntil(char *target, char *terminator); // as find but search ends
    ...
    size_t readBytes(char *buffer, size_t length); // read chars from stream
                                                   // into buffer
    size_t readBytes(uint8_t *buffer, size_t length) {
        return readBytes((char *)buffer, length);
    }
    // terminates if length characters have been read or timeout (see
    // setTimeout) returns the number of characters placed in the buffer (0
    // means no valid data found)

    size_t readBytesUntil(char   terminator,
                          char  *buffer,
                          size_t length); // as readBytes with terminator
                                          // character
