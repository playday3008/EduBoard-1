...
void doSomething(void *context) {
  int time = (int)context; // the number '1' was passed to this method, and are
                           // stored in 'context'
  Serial.print(time);
...
void doAfter(void *context) {
  int time = (int)context; // the number '10' was passed to this method, and are
                           // stored in 'context'
  Serial.print("Stopping the LED event after ");
  Serial.print(time);
  Serial.println(" seconds.");
  t.stop(ledEvent);                    // Stops the ledEvent
  t.oscillate(ledPin, pause4, LOW, 5); // (pin, pause, start condition, number
                                       // of oscillations) - oscillates the
                                       // ledPin 5 times with start condition
                                       // LOW
}
